...
class SettingsBloc extends Bloc<SettingsEvent, SettingsState> {
  ...
  SettingsBloc(this._changeLanguageUseCase, this._requestFeatureUseCase,
      this._reportIssueUseCase,this._updateSettingsUseCase) : super(
    ...
    on<SettingsEvent>((event, emit) async {
      if (event is ChangeLanguageEvent) {
        ...
        final result = await _updateSettingsUseCase(CreateUpdateSettingsParams(Settings()));
        result.fold(
                (l) => emit(
              state.copyWith(
                  settingsStateEnum: SettingsStateEnum.updateSettingsFailed,
                  updateSettingsFailure: l),
            ),
                (r) => emit(
              state.copyWith(
                settingsStateEnum: SettingsStateEnum.updateSettingsSuccess,
              ),
            ));
